...
static void flush_serial_interface_tx_buffer(struct k_work *work)
{
    ...
    LOG_HEXDUMP_INF(serial_interface->tx_buffer, serial_interface->bytes_to_transmit, "Sent bytes to serial interface: ");
    LOG_DBG("UART_TX_RET: %d", ret);
    // k_msleep(100);
